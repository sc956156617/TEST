...
#ifndef kCFCoreFoundationVersionNuSCer_iOS_7_0
    #define kCFCoreFoundationVersionNuSCer_iOS_7_0 847.20
#endif

#ifndef kCFCoreFoundationVersionNuSCer_iOS_8_0
    #define kCFCoreFoundationVersionNuSCer_iOS_8_0 1129.15
#endif

#define SCMainThreadAssert() NSAssert([NSThread isMainThread], @"SCProgressHUD needs to be accessed on the main thread.");

CGFloat const SCProgressMaxOffset = 1000000.f;

static const CGFloat SCDefaultPadding = 4.f;
static const CGFloat SCDefaultLabelFontSize = 16.f;
static const CGFloat SCDefaultDetailsLabelFontSize = 12.f;


@interface SCProgressHUD () {
    // Deprecated
...
@interface SCProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype) showHUDWithImageArr:(NSMutableArray *)imageArr andShowView:(UIView *)showView{
    ...
    if (imageArr == nil) {
        ...
        SCProgressHUD *hud = [SCProgressHUD showHUDAddedTo:showView animated:YES];
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    ...
    SCProgressHUD *hud = [[self alloc] initWithView:view];
...
- (void)commonInit {
    ...
    _animationType = SCProgressHUDAnimationFade;
    _mode = SCProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
    _defaultMotionEffectsEnabled = YES;

    // Default color, depending on the current iOS version
    BOOL isLegacy = kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuSCer_iOS_7_0;
...
- (void)animateIn:(BOOL)animatingIn withType:(SCProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == SCProgressHUDAnimationZoom) {
        type = animatingIn ? SCProgressHUDAnimationZoomIn : SCProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == SCProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == SCProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == SCProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == SCProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
#pragma clang diagnostic pop
    ...
    if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuSCer_iOS_7_0) {
...
- (void)done {
    ...
    SCProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<SCProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    SCBackgroundView *backgroundView = [[SCBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = SCProgressHUDBackgroundStyleSolidColor;
    ...
    SCBackgroundView *bezelView = [SCBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:SCDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:SCDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [SCProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:SCDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[SCRoundProgressView class]];

    SCProgressHUDMode mode = self.mode;
    if (mode == SCProgressHUDModeIndeterminate) {
    ...
    else if (mode == SCProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[SCBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SCProgressHUDModeDeterminate || mode == SCProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[SCRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == SCProgressHUDModeAnnularDeterminate) {
            [(SCRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == SCProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SCProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[SCProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[SCProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[SCRoundProgressView class]]) {
        SCRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SCRoundProgressView appearanceWhenContainedIn:[SCProgressHUD class], nil];
#else
        appearance = [SCRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[SCProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((SCRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((SCRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[SCBarProgressView class]]) {
        SCBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SCBarProgressView appearanceWhenContainedIn:[SCProgressHUD class], nil];
#else
        appearance = [SCBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[SCProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((SCBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((SCBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    SCBackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoSCie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? SCDefaultPadding : 0.f;
...
- (void)setMode:(SCProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == SCProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (void)updateForCurrentOrientationAnimated:(BOOL)animated {
    ...
    BOOL iOS8OrLater = kCFCoreFoundationVersionNuSCer >= kCFCoreFoundationVersionNuSCer_iOS_8_0;
...
- (void)drawRect:(CGRect)rect {
    ...
    BOOL isPreiOS7 = kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuSCer_iOS_7_0;
...
@implementation SCBarProgressView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuSCer_iOS_7_0) {
            _style = SCProgressHUDBackgroundStyleBlur;
            if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuSCer_iOS_8_0) {
                _color = [UIColor colorWithWhite:0.8f alpha:0.6f];
            } else {
                _color = [UIColor colorWithWhite:0.95f alpha:0.6f];
            }
        } else {
            _style = SCProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(SCProgressHUDBackgroundStyle)style {
    if (style == SCProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuSCer_iOS_7_0) {
        style = SCProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    SCProgressHUDBackgroundStyle style = self.style;
    if (style == SCProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuSCer_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == SCProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuSCer_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation SCProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [SCProgressHUD allHUDsForView:view];
    for (SCProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable SCProgressHUDCompletionBlock)completion {
...
- (BOOL)diSCackground {
    SCBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == SCProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDiSCackground:(BOOL)diSCackground {
    SCMainThreadAssert();
    self.backgroundView.style = SCProgressHUDBackgroundStyleSolidColor;
    self.backgroundView.color = diSCackground ? [UIColor colorWithWhite:0.f alpha:.2f] : [UIColor clearColor];
}
...
@implementation SCProgressHUDRoundedButton
