...
static JZLNetworkingTool *_instance = nil;
...
{
    // /先导入证书
    NSString *cerPath = [[NSBundle mainBundle] pathForResource:certificater ofType:@"cer"]; //证书的路径
...
+ (instancetype)sharedManager {
    ...
    dispatch_once( &onceToken, ^{
        ...
        _instance = [[JZLNetworkingTool alloc] initWithBaseURL:baseUrl];
        // https ssl 验证。
            if(openHttpsSSL)
            {
...
+ (void)getWithUrl: (NSString *)url params: (NSDictionary *)params isReadCache: (BOOL)isReadCache success: (responseSuccess)success failed: (responseFailed)failed  {
  [[JZLNetworkingTool sharedManager] GET:url parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
      //请求成功的回调
      if (success) {
...
+ (void)postWithUrl:(NSString *)url params:(NSDictionary *)params isReadCache: (BOOL)isReadCache success:(responseSuccess)success failed:(responseFailed)failed {
    ...
    [[JZLNetworkingTool sharedManager]    POST:url parameters:params  success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
        if (success) {
                        success(task,responseObject);
                    }
                    //请求成功,保存数据
