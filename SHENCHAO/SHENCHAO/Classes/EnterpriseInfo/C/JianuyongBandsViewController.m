...
@interface JianuyongBandsViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation JianuyongBandsViewController
...
- (void)viewDidLoad {
    ...
    [JZLNetworkingTool getWithUrl:BandsList params:nil isReadCache:YES success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        for (NSDictionary *  dic in responseObject[@"carModelJson"]) {
...
-(void)backClick{
    //通过归档将模型数组写入本地文件中
    [NSArray saveArrayData:self.dataSource andSaveName:@"jianying.pl"];
...
-(NSArray *)dataSource{
    if (_dataSource ==nil) {
        NSArray *arrayDict=[NSArray getArraySaveName:@"jianying.pl"];
        if (arrayDict.count) {
