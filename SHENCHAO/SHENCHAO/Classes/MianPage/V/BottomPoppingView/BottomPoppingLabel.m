...
@implementation BottomPoppingLabel
...
//    _textLabel.frame = CGRectMake(self.imageEdgeInsets.left, self.imageEdgeInsets.top, sideLength, sideLength);
...
- (void)setModel:(LableModel *)model {
    _textLabel.text = model.text;
}

- (void)updateLayoutBySize:(CGSize)size finished:(void (^)(BottomPoppingLabel *))finished {
    CGRect frame = self.frame;
    frame.size = size;
    self.frame = frame;
...
+ (instancetype)modelWithTitle:(NSString *)title {
    LableModel *model = [LableModel new];
    model.text = title;
    return model;
}
