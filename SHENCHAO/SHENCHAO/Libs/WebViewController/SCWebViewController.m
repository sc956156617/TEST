...
#define Support @"董鹏工作室"
#define BackgroundColor  [UIColor grayColor]
#define  SupportLabelTextColor [UIColor whiteColor]
@interface SCWebViewController ()<UIWebViewDelegate,UIActionSheetDelegate,WKNavigationDelegate,WKUIDelegate,UIScrollViewDelegate,GoToInfoViewControllerProtocol>
/** 进度条 */
...
#pragma mark - ***** 判断是否有关闭按钮
-(void)checkIsHaveCloseItem{
    if (_isWKWebView) {
        if (self.wkWebView.canGoBack) {
            if (self.navigationItem.leftBarButtonItems.count == 1) {
                [self configCloseItem];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context{
    if (object == self.wkWebView && [keyPath isEqualToString:@"estimatedProgress"]) {
        ...
        NSLog(@"%f",newprogress);
        if (newprogress < 1.0 && _isHiddenProgressView == NO) {
            self.progressView.hidden = NO;
...
#pragma mark 实现代理交互方法
-(void)zttDelegateWith:(NSString *)jiemian{
    
    //    [MBProgressHUD showText:@"图片已保存到本地相册" inview:self.view];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (_webviewHeaderType == SCWebviewHeaderTypeSupportLabel ) {
...
- (UILabel *)supportLabel{
    if (_supportLabel == nil) {
        ...
        _supportLabel.textColor = SupportLabelTextColor;
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (_webviewHeaderType == SCWebviewHeaderTypeRefreshHeader) {
