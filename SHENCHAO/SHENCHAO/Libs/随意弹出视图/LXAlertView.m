...
-(instancetype)initWithTitle:(NSString *)title textView:(NSString *)placeholdText cancelBtnTitle:(NSString *)cancelTitle otherBtnTitle:(NSString *)otherBtnTitle clickIndexBlock:(LXAlertClickIndexBlock)block{
    if(self=[super init]){
        ...
        _textField.placeholder=placeholdText;
        _textField.textColor=[UIColor lightGrayColor];
        _textField.font=LXADMessageFont;
//        _textField.numberOfLines=0;
        _textField.textAlignment=NSTextAlignmentCenter;
        _textField.layer.borderColor = SCThemeColor.CGColor;
        _textField.layer.cornerRadius = 5;
        _textField.layer.borderWidth  = .5;
//        _textField.lineBreakMode=NSLineBreakByTruncatingTail;
//        _textField.characterSpace=2;
//        _textField.lineSpace=3;
//        CGSize labSize = [_textField getLableRectWithMaxWidth:AlertView_W-messageLabSpace*2];
        CGFloat messageLabAotuH =  MessageMin_H ;
        CGFloat endMessageLabH = messageLabAotuH > MessageMAX_H?MessageMAX_H:messageLabAotuH;
        _textField.frame=CGRectMake(messageLabSpace, _titleLab.frame.size.height+_titleLab.frame.origin.y+10, AlertView_W-messageLabSpace*2, endMessageLabH);
        
        
        //计算_alertView的高度
        _alertView.frame=CGRectMake(0, 0, AlertView_W, _textField.frame.size.height+LXATitle_H+LXABtn_H+40);
...
-(void)btnClick:(UIButton *)btn{
    ...
    if (self.clickBlock) {
        self.clickBlock(btn.tag,_textField.text?_textField.text:@"");
    }
