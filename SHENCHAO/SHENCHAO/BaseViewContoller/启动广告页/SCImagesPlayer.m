...
@interface SCImagesPlayer ()<UICollectionViewDelegate, UICollectionViewDataSource>
...
- (void)setIndicatorPattern:(id<SCImagesPlayerIndictorPattern>)indicatorPattern
{
    _indicatorPattern = indicatorPattern;
    if (indicatorPattern) {
        [self.indicatorView removeFromSuperview];
        [self layoutIfNeeded];
        [self addSubview:[self.indicatorPattern indicatorViewInSCImagesPlayer:self]];
    }
}
...
static NSString * const reuseIdentifier = @"SCImagesPlayerCell";
//添加collectionView
...
{
    [SCImagesPlayer checkElementOfImages:images];
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //通知代理更新自定义分页指示器
    if ([self.indicatorPattern respondsToSelector:@selector(SCImagesPlayer:didChangedIndex:count:)]) {
        [self.indicatorPattern SCImagesPlayer:self didChangedIndex:indexPath.item % imageCount + 1 count:imageCount];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(SCImagesPlayer:didSelectImageAtIndex:)]) {
        [self.delegate SCImagesPlayer:self didSelectImageAtIndex:(indexPath.item % imageCount)];
    }
